import {
  ...
  async function login() {
    if (!session.info.isLoggedIn) {
        const { redirectUrl } = await chrome.runtime.sendMessage({messageType: "requestRedirectUrl"});
      ...
      await session.login({
        ...
        redirectUrl,
        handleRedirect: async (redirectUrl) => {
          const response = await chrome.runtime.sendMessage({messageType: 'startLoginFlow', redirectUrl});
          handleRedirectAfterLogin(response);
          alert(response);
        }
      });
    }
  ...
  async function handleRedirectAfterLogin(url) {
    window.history.replaceState = () => {}
    await session.handleIncomingRedirect(url || window.location.href);
    if (session.info.isLoggedIn) {
